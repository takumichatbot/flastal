...
function LoginContent() {
  ...
  return (
    <div className="bg-white max-w-md w-full p-8 border border-gray-100 rounded-2xl shadow-xl animate-fadeIn">
      {/* ヘッダー部分 */}
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <button 
          type="submit" 
          disabled={isLoading}
          className={`w-full py-3.5 bg-sky-500 text-white rounded-lg font-bold text-lg shadow-md hover:bg-sky-600 hover:shadow-lg transition-all duration-200 transform hover:-translate-y-0.5 flex justify-center items-center gap-2 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
        >
      ...
      {/* 再送信エリア */}
      {showResend && (
          <div className="mt-6 p-4 bg-orange-50 border border-orange-200 rounded-xl text-center animate-bounce-subtle">
...
export default function LoginPage() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 to-white flex items-center justify-center p-4">
      {/* Suspense境界を置くことで、ビルドエンジン（Prerender）に対して 
        LoginContent内の動的フック(router, auth等)を後回しにするよう指示します 
      */}
      <Suspense fallback={
        <div className="flex flex-col items-center justify-center p-12 bg-white rounded-2xl shadow-xl w-full max-w-md border border-gray-100">
          <FiLoader className="w-10 h-10 text-sky-500 animate-spin mb-4" />
          <p className="text-slate-400 text-sm font-medium tracking-wider uppercase">Loading System...</p>
        </div>
      }>
        <LoginContent />
      </Suspense>
