...
function LoginForm() {
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Apple認証に失敗しました');

      await login(data.token, data.user, data.refreshToken);
      toast.success('ログインしました！');
      router.push('/mypage');
    } catch (err) {
      if (err?.message !== 'The user closed the native Apple Sign In flow.') {
        toast.error(err.message || 'Apple Sign In に失敗しました');
      }
    } finally {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 flex-1">
          ...
          {isNativeApp && (
            <motion.button
              ...
              className="w-full py-3.5 bg-black text-white rounded-2xl font-black text-base flex items-center justify-center gap-3 transition-all disabled:opacity-60"
            >
              {isAppleLoading ? (
              ...
              {isAppleLoading ? '認証中...' : 'Appleでサインイン'}
            </motion.button>
          )}
