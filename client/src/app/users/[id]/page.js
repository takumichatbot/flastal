...
function ProjectCard({ project, delay = 0 }) {
  ...
  const s = statusLabels[project.status] || { text: project.status, color: 'bg-slate-400' };
      ...
      <Link href={`/projects/${project.id}`} className="block active:opacity-70 transition-opacity">
        <div className="relative aspect-square bg-slate-100 rounded-2xl overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex flex-col justify-end p-2.5">
            <span className={`text-[9px] text-white px-1.5 py-0.5 rounded-full w-fit mb-1 font-black ${s.color}`}>{s.text}</span>
            <p className="text-white text-[11px] font-bold line-clamp-2 leading-tight">{project.title}</p>
          </div>
        </div>
      </Link>
    </motion.div>
...
export default function PublicUserProfile() {
  ...
  const TABS = [
    ...
    { id: 'backed',  label: '参加', icon: Heart, count: backedCount },
    { id: 'album',   label: 'アルバム', icon: Grid3X3, count: null },
  ];

  return (
    <div className="flex flex-col bg-slate-50 font-sans" style={{ minHeight: '100dvh' }}>
      ...
      <div className="fixed top-0 left-0 right-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-100">
        ...
        >
          ...
          <p className="flex-1 text-center font-black text-slate-800 text-base">{profile.handleName}</p>
          <div className="w-9" />
        </div>
      </div>
      ...
      <div
        ...
        style={{ paddingTop: 'calc(3.5rem + env(safe-area-inset-top))', paddingBottom: 'calc(5rem + env(safe-area-inset-bottom))' }}
      >
        {/* プロフィールカード */}
        <div className="px-4 pt-6 pb-4">
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6">
            <div className="flex items-center gap-4 mb-5">
              <div className="w-20 h-20 rounded-full overflow-hidden bg-slate-100 border-2 border-white shadow-md flex-shrink-0">
                {profile.iconUrl ? (
                  <Image src={profile.iconUrl} alt={profile.handleName} width={80} height={80} className="object-cover w-full h-full" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                <h1 className="text-xl font-black text-slate-800 truncate">{profile.handleName}</h1>
                {/* SNS links */}
                <div className="flex items-center gap-2 mt-2">
                  {profile.twitterUrl && (
                    <a href={profile.twitterUrl} target="_blank" rel="noopener noreferrer"
                      className="w-8 h-8 bg-slate-50 rounded-full flex items-center justify-center text-slate-400 active:bg-slate-100 transition-colors">
                  ...
                  {profile.instagramUrl && (
                    <a href={profile.instagramUrl} target="_blank" rel="noopener noreferrer"
                      className="w-8 h-8 bg-slate-50 rounded-full flex items-center justify-center text-pink-400 active:bg-slate-100 transition-colors">
                      <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 rounded-2xl p-3.5 text-center">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mt-0.5">主催した企画</p>
              </div>
              <div className="bg-slate-50 rounded-2xl p-3.5 text-center">
                <p className="text-2xl font-black text-slate-800">{backedCount}</p>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mt-0.5">参加した企画</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {TABS.map(tab => {
            ...
            return (
              <button
                ...
                className={`flex-1 flex flex-col items-center py-3 gap-1 transition-colors relative ${active ? 'text-pink-500' : 'text-slate-400'}`}
              >
                ...
                {active && (
                  <motion.div
                    ...
                    className="absolute bottom-0 left-4 right-4 h-0.5 bg-pink-500 rounded-full"
                  />
                )}
              </button>
            );
            ...
            >
              {backedCount > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-slate-300">
                  ...
                  <p className="text-sm font-bold text-slate-400">まだ参加した企画はありません</p>
                </div>
              )}
            </motion.div>
          )}
