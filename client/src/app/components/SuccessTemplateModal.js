...
import { CheckCircle2, Search, Crown, LayoutTemplate, X, TrendingUp } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 animate-fadeIn">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden transform transition-all scale-100">
        ...
        {/* ヘッダー */}
        <div className="bg-gradient-to-r from-pink-500 to-rose-600 p-6 text-white shrink-0">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xl font-bold flex items-center gap-2">
                <Crown className="text-yellow-300 text-2xl" /> 成功企画テンプレート
              </h2>
          ...
          <div className="mt-6 relative group">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-pink-200 group-focus-within:text-white transition-colors" />
            <input 
              type="text"
              placeholder="キーワードで検索 (例: 生誕祭, フラスタ, 楽屋花...)"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-white/10 border border-white/20 text-white placeholder-pink-200/70 rounded-lg pl-10 pr-4 py-2.5 focus:bg-white/20 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all text-sm"
            />
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-400">
               <div className="bg-slate-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                   <LayoutTemplate className="text-3xl opacity-50" />
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredTemplates.map((template) => {
                ...
                return (
                  <div 
                    key={template.id} 
                    className="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-xl hover:-translate-y-1 hover:border-pink-300 transition-all duration-300 group cursor-pointer flex flex-col h-full relative overflow-hidden"
                    onClick={() => onSelect(template)}
                  >
                    {/* 背景装飾 */}
                    <div className="absolute top-0 right-0 p-2 opacity-10 group-hover:opacity-20 transition-opacity">
                        <Crown size={60} className="text-pink-500 transform rotate-12"/>
                    </div>
                    ...
                    <h3 className="font-bold text-slate-800 mb-2 line-clamp-2 group-hover:text-pink-600 transition-colors relative z-10">
                        {template.title}
                    </h3>
                    ...
                    <div className="bg-slate-50 rounded-lg p-3 space-y-2 mb-4 border border-slate-100 relative z-10">
                        ...
                        <div className="flex justify-between text-xs">
                            ...
                            <span className="font-bold text-pink-600">{template.totalPledged.toLocaleString()} pt</span>
                        </div>
                        {/* プログレスバー */}
                        ...
                        <div className="text-right text-[10px] text-pink-600 font-bold">
                            達成率 {rawPercentage.toFixed(0)}%
                        </div>
                    </div>

                    <button 
                        className="w-full py-2.5 text-sm font-bold text-white bg-pink-500 rounded-lg shadow-md group-hover:bg-pink-600 group-hover:shadow-lg transition-all flex items-center justify-center gap-2 relative z-10"
                    >
                        <CheckCircle2 /> この構成を使う
                    </button>
                  </div>
                );
              })}
