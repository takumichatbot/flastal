...
import { 
  ...
  CreditCard, Lock, Smartphone, Megaphone, Info, MousePointer2
} from 'lucide-react';
...
const MagicCursor = () => {
  ...
  return (
    <motion.div
      className="fixed top-0 left-0 w-8 h-8 rounded-full bg-gradient-to-r from-pink-400 to-sky-400 blur-xl pointer-events-none z-50 opacity-50 hidden md:block"
...
const ScrollProgress = () => {
  ...
  return (
    <motion.div className="fixed top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-pink-500 via-purple-500 to-sky-500 origin-left z-[60] shadow-[0_0_15px_rgba(236,72,153,0.6)]" style={{ scaleX }} />
  );
};

// 3. ふわっと現れるラッパー (Stagger)
...
// 5. 背景で動く図形 (Parallax)
const FloatingShape = ({ color, top, left, right, bottom, size, speed = 1 }) => {
  const { scrollY } = useScroll();
  const y = useTransform(scrollY, [0, 1000], [0, speed * 150]);
  
  return (
    <motion.div 
      style={{ top, left, right, bottom, width: size, height: size, y }}
      className={cn("absolute rounded-full blur-3xl opacity-40 pointer-events-none mix-blend-multiply z-0", color)}
      animate={{ 
        scale: [1, 1.1, 0.9, 1],
        rotate: [0, 20, -20, 0]
      }}
...
const SectionHeader = ({ en, ja, desc, color = "pink" }) => {
  ...
  return (
    <div className="text-center mb-20 relative z-10">
      <Reveal>
        <span className={cn("inline-block font-bold tracking-[0.2em] uppercase text-sm mb-3 font-mono px-3 py-1 rounded-full bg-white/50 backdrop-blur border border-white/50 shadow-sm", colors[color].split(" ").pop())}>
          ✨ {en}
        </span>
        <h2 className="text-3xl md:text-5xl font-black text-slate-800 mb-6 leading-tight">
          {ja}
        </h2>
        <div className={cn("h-1.5 w-24 mx-auto rounded-full bg-gradient-to-r mb-6 shadow-sm", colors[color].split(" ")[0] + " " + colors[color].split(" ")[1])} />
        {desc && (
...
const KawaiiButton = ({ children, variant = "primary", icon: Icon, className, onClick }) => {
  const baseStyle = "relative px-8 py-4 rounded-full font-bold text-lg shadow-xl transition-all overflow-hidden group flex items-center justify-center gap-3 active:scale-95";
  const variants = {
  ...
  return (
    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={cn(baseStyle, variants[variant], className)} onClick={onClick}>
...
const HeroSection = () => {
  return (
    <section className="relative w-full min-h-[95vh] flex items-center justify-center overflow-hidden bg-slate-50">
      ...
      <FloatingShape color="bg-pink-300" top="-10%" left="-10%" size={500} speed={0.5} />
      <FloatingShape color="bg-sky-300" bottom="-10%" right="-10%" size={500} speed={-0.5} />
      <FloatingShape color="bg-yellow-200" top="40%" right="10%" size={300} speed={0.2} />
        ...
        <div className="lg:col-span-7 text-center lg:text-left">
          ...
          <Reveal delay={0.1}>
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black text-slate-800 leading-[1.1] mb-8 tracking-tight">
              ...
              <span className="relative inline-block group">
                <span className="absolute inset-0 bg-pink-200 -rotate-2 rounded-lg blur-sm opacity-50 transform group-hover:scale-110 transition-transform" />
          ...
          <Reveal delay={0.2}>
            <p className="text-lg md:text-xl text-slate-600 mb-10 leading-relaxed max-w-2xl mx-auto lg:mx-0">
              ...
              <span className="font-bold text-slate-800 bg-pink-50 px-2 py-0.5 rounded-md mx-1 shadow-sm">安全</span>
              <span className="font-bold text-slate-800 bg-sky-50 px-2 py-0.5 rounded-md mx-1 shadow-sm">簡単</span>
              <span className="font-bold text-slate-800 bg-yellow-50 px-2 py-0.5 rounded-md mx-1 shadow-sm">感動的</span>
              にする<br/>
        ...
        <div className="lg:col-span-5 relative h-[500px] lg:h-[600px] hidden md:block perspective-1000">
          <TiltCard className="w-full h-full">
            <motion.div 
              ...
              className="relative w-full h-full bg-white rounded-[40px] shadow-2xl border-[6px] border-white overflow-hidden transform"
            >
            ...
            <motion.div 
              ...
              className="absolute -top-6 -right-6 bg-white p-4 rounded-2xl shadow-xl flex items-center gap-3 z-30 border border-slate-50"
            >
...
const CultureSection = () => {
  const cultures = [
    { title: "フラワースタンド", eng: "Flower Stand", icon: "💐", desc: "ライブ会場のロビーを彩る、背の高いお花。バルーンやLEDで派手に装飾するのが定番。" },
    { title: "楽屋花（卓上）", eng: "Arrangement", icon: "🧺", desc: "楽屋や受付に飾るコンパクトなお花。スタンド禁止の会場や、個人的に想いを伝えたい時に。" },
    { title: "イラストパネル", eng: "Illustration Panel", icon: "🎨", desc: "神絵師に依頼した推しのイラストをパネル化してお花に添えます。二次元界隈では必須！" },
    { title: "祭壇・デコ", eng: "Altar & Decor", icon: "🧸", desc: "ぬいぐるみ、グッズ、名札パネルなどを大量に盛り込んだ、愛の重さが伝わる独自のデザイン。" }
  ];
          ...
          {cultures.map((c, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <TiltCard className="h-full">
                <div className="bg-slate-50 rounded-[30px] p-6 text-center border border-slate-100 group h-full flex flex-col items-center">
                  <div className="text-6xl mb-6 transform group-hover:scale-125 transition-transform duration-300 drop-shadow-md">{c.icon}</div>
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed text-left bg-white p-4 rounded-xl shadow-inner">
                    {c.desc}
                  </p>
                </div>
              </TiltCard>
          ...
          <Reveal>
            <div className="bg-white p-10 rounded-[40px] border border-slate-200 grayscale opacity-70 hover:grayscale-0 hover:opacity-100 transition-all duration-500">
              <h3 className="text-xl font-bold text-slate-600 mb-8 flex items-center gap-3">
                <span className="bg-slate-200 px-3 py-1 rounded-full text-xs">従来のやり方</span>
                😰 大変すぎる...
              </h3>
              <ul className="space-y-6">
                {["DMで一人ひとり口座を教える手間", "未入金の催促が気まずい", "本名や住所がバレるリスク", "収支報告のエクセル管理が地獄"].map((item, i) => (
                  <li key={i} className="flex gap-4 text-slate-500 items-center">
                    <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0 text-slate-400 font-bold">×</div>
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <Reveal delay={0.2}>
            <TiltCard>
              <div className="bg-gradient-to-br from-white to-sky-50 p-10 rounded-[40px] border-4 border-sky-100 shadow-2xl relative overflow-hidden group">
                ...
                <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-sky-200 rounded-full blur-3xl opacity-50 group-hover:scale-150 transition-transform duration-700" />
                
                <h3 className="text-2xl font-bold text-sky-600 mb-8 flex items-center gap-3">
                  <span className="bg-sky-100 px-3 py-1 rounded-full text-xs">FLASTALなら</span>
                  ✨ 全部おまかせ！
                </h3>
                <ul className="space-y-6 relative z-10">
                  {["リンクをシェアするだけで集金完了", "クレカ・コンビニ払いで自動管理", "完全匿名で安心安全", "収支報告もワンクリックで公開"].map((item, i) => (
                    <li key={i} className="flex gap-4 text-slate-700 font-bold items-center">
                      <div className="w-8 h-8 rounded-full bg-sky-500 text-white flex items-center justify-center shrink-0 shadow-lg shadow-sky-200">
            ...
            <Reveal className="h-full">
              <TiltCard className="h-full">
                <div className="h-full bg-gradient-to-br from-slate-50 to-white border border-slate-200 rounded-[40px] p-10 flex flex-col relative overflow-hidden group hover:border-purple-200 transition-colors">
                  <div className="absolute top-0 right-0 p-10 opacity-10 group-hover:opacity-20 transition-opacity transform rotate-12">
                    <CreditCard size={250} />
                  </div>
          ...
          <Reveal delay={0.2} className="h-full">
            <TiltCard className="h-full">
              <div className="h-full bg-sky-50 rounded-[40px] p-8 relative overflow-hidden group border border-sky-100">
                ...
                <div className="bg-white w-12 h-12 rounded-2xl flex items-center justify-center text-sky-500 mb-4 shadow-sm">
          ...
          <Reveal delay={0.3} className="h-full">
            <TiltCard className="h-full">
              <div className="h-full bg-pink-50 rounded-[40px] p-8 relative overflow-hidden group border border-pink-100">
                ...
                <div className="bg-white w-12 h-12 rounded-2xl flex items-center justify-center text-pink-500 mb-4 shadow-sm">
                  <Flower size={24} />
                </div>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <Reveal>
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl">
              <h3 className="font-bold text-xl mb-8 flex items-center gap-3 border-b border-slate-100 pb-4">
                <Flower className="text-pink-500" /> 提携フラワーショップ
              </h3>
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <motion.div key={i} whileHover={{ x: 10 }} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-pink-50 transition-colors cursor-pointer group">
                    <div className="w-12 h-12 bg-slate-200 rounded-full overflow-hidden shrink-0 group-hover:scale-110 transition-transform">
              ...
              <div className="mt-6 text-right">
                <Link href="/florists" className="text-sm text-pink-500 font-bold hover:underline flex items-center justify-end gap-1">もっと見る <ArrowRight size={14}/></Link>
              </div>
            </div>
          </Reveal>
          ...
          <Reveal delay={0.2}>
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl">
              ...
              <div className="space-y-4">
                 {[1, 2, 3].map((i) => (
                  <motion.div key={i} whileHover={{ x: 10 }} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-sky-50 transition-colors cursor-pointer group">
                    <div className="w-12 h-12 bg-slate-200 rounded-full overflow-hidden shrink-0 group-hover:scale-110 transition-transform">
                      <div className="w-full h-full bg-slate-300 animate-pulse" />
                    </div>
                    <div>
               ...
               <div className="mt-6 text-right">
                <Link href="/creators" className="text-sm text-sky-500 font-bold hover:underline flex items-center justify-end gap-1">もっと見る <ArrowRight size={14}/></Link>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <div className="flex gap-8 overflow-x-auto px-6 pb-12 snap-x items-center">
        {[1,2,3,4,5].map((i) => (
          <motion.div 
            ...
            className="snap-center shrink-0 w-[300px] md:w-[400px] bg-slate-800 rounded-[30px] overflow-hidden shadow-2xl border border-slate-700 group cursor-pointer"
          >
             ...
             <div className="p-6">
               <div className="flex justify-between text-sm text-slate-400 mb-4 font-mono">
                 <span>Total: ¥240,000</span>
                 <span>Fans: 85</span>
               </div>
  ...
  return (
    <section className="py-32 bg-white">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            { text: "初めての主催で不安でしたが、集金管理が自動なのでデザインの相談に集中できました。お花屋さんも紹介してもらえて助かりました！", name: "A.Sさん", role: "VTuberファン / 主催歴1回", bg: "bg-pink-100" },
            { text: "たった1000円からの支援でも、名前をパネルに載せてもらえて嬉しかったです。現地に行けなくても「推し活」に参加できた実感がありました。", name: "T.Kさん", role: "アイドルファン / 参加者", bg: "bg-sky-100" },
            { text: "イラストのデータを共有する機能が便利でした。解像度の問題とかも事前にチェックしてくれたので、印刷も綺麗にいきました。", name: "M.Mさん", role: "声優ファン / 絵師依頼", bg: "bg-purple-100" }
          ].map((v, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <div className="bg-slate-50 p-8 rounded-[30px] relative border border-slate-100 h-full flex flex-col hover:shadow-lg transition-shadow">
                ...
                <p className="text-slate-600 text-sm leading-relaxed mb-6 pt-6 relative z-10 font-medium">
                  {v.text}
                </p>
          ...
          {steps.map((step, i) => (
            <Reveal key={i} width="100%">
              <div className={cn("relative flex items-center mb-16 last:mb-0", i % 2 === 0 ? "md:justify-start" : "md:justify-end")}>
                <div className="absolute left-4 md:left-1/2 -translate-x-1/2 w-10 h-10 rounded-full bg-white border-4 border-sky-400 z-10 flex items-center justify-center text-sm font-bold text-sky-500 shadow-md">
                  {i + 1}
                </div>
      ...
      <div className="container mx-auto px-6 text-center relative z-10">
        <Reveal>
          ...
          <div className="flex justify-center gap-6 opacity-30">
            ...
            <div className="w-16 h-16 bg-white/10 rounded-2xl animate-pulse delay-75" />
          </div>
        </Reveal>
      </div>
    </section>
      ...
      <div className="container mx-auto px-6 max-w-3xl">
        ...
        <div className="space-y-4">
          {[
            ...
            { q: "当日、現地に行けなくても参加できますか？", a: "もちろんです！完成したフラスタの写真はサイト上で高画質で共有されます。" }
          ].map((item, i) => (
            <Reveal key={i} delay={i * 0.05}>
              <details className="group bg-slate-50 rounded-2xl p-6 border border-slate-100 cursor-pointer [&_summary::-webkit-details-marker]:hidden open:bg-white open:shadow-lg open:border-emerald-100 transition-all duration-300">
                ...
                <motion.div 
                  ...
                  className="mt-4 text-sm text-slate-600 pl-9 leading-relaxed"
                >
                  {item.a}
                </motion.div>
              </details>
    ...
    <section className="py-24 bg-slate-50">
      <div className="container mx-auto px-6 max-w-4xl">
         <div className="flex justify-between items-end mb-8">
           ...
           <Link href="/news" className="text-sm text-pink-500 font-bold hover:underline">一覧を見る</Link>
         </div>
      ...
      <div className="container mx-auto px-6 text-center relative z-10">
        <Reveal>
          <div className="bg-gradient-to-r from-pink-500 via-rose-500 to-pink-600 rounded-[60px] p-12 md:p-24 text-white shadow-2xl shadow-pink-200 relative overflow-hidden group">
            ...
            <FloatingShape color="bg-yellow-300" bottom="-20%" left="-10%" size={300} delay={2} />
            ...
            <div className="relative z-10">
              ...
              <p className="text-pink-100 text-lg mb-12 max-w-xl mx-auto font-medium">
                企画の作成は無料です。<br/>
              ...
              <Link href="/create">
                <motion.button 
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="bg-white text-pink-600 px-16 py-6 rounded-full text-2xl font-bold shadow-xl hover:shadow-2xl transition-all relative overflow-hidden group"
                >
