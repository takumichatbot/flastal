...
const TYPE_CONFIG = {
  USER: {
    label: 'ファン', themeColor: 'pink', iconBg: 'bg-pink-100', iconColor: 'text-pink-600',
    btnBg: 'bg-pink-500 hover:bg-pink-600', linkText: 'text-pink-600', loginLink: '/login',
  },
  FLORIST: {
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              {errorMessage && <div className="text-red-500 text-xs mt-2 font-bold"><AlertCircle className="inline mr-1"/>{errorMessage}</div>}
            </div>
            <button disabled={status === 'loading'} className={`w-full py-3.5 text-white font-bold rounded-xl shadow-md flex items-center justify-center ${config.btnBg}`}>
              {status === 'loading' ? <Loader2 className="animate-spin mr-2" /> : '再設定メールを送信'}
            </button>
          </form>
          <div className="text-center mt-6 pt-6 border-t">
            <Link href={config.loginLink} className={`text-sm font-bold flex items-center justify-center gap-1 ${config.linkText}`}><ArrowLeft /> ログインページに戻る</Link>
          </div>
        </>
      )}
    </div>
...
export default function ForgotPasswordPage() {
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <Suspense fallback={<Loader2 className="animate-spin text-gray-400" size={40} />}>
        <ForgotPasswordForm />
      </Suspense>
    </div>
  );
