...
export default function EditProjectPage() {
  ...
  const { user, token, loading: authLoading } = useAuth(); // ★tokenを取得
  ...
  const handleSubmit = async (e) => {
    ...
    const promise = fetch(`${API_URL}/api/projects/${projectId}`, { 
      ...
      body: JSON.stringify(formData), // userIdは送らない
    }).then(async res => {
      if (!res.ok) {
      ...
      <div className="max-w-2xl mx-auto p-8 bg-white rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {formData.imageUrl && (
                <div className="relative w-full h-48 my-2 rounded-md overflow-hidden bg-gray-100">
                    <Image src={formData.imageUrl} alt="プレビュー" fill className="object-cover" />
                </div>
            )}
            <input type="file" onChange={handleImageUpload} disabled={isUploading} className="text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-sky-100 file:text-sky-700 hover:file:bg-sky-200 cursor-pointer" />
          </div>
          ...
          <div className="flex gap-4 pt-4">
            <Link href={`/projects/${projectId}`} className="flex-1 text-center px-4 py-3 font-semibold text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors">
                キャンセル
            </Link>
            <button type="submit" disabled={isSubmitting || isUploading} className="flex-1 px-4 py-3 font-semibold text-white bg-green-500 rounded-lg hover:bg-green-600 disabled:bg-slate-300 disabled:cursor-not-allowed transition-colors">
