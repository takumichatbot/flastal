    ...
    <AppCard className="border-none shadow-[0_10px_30px_rgba(244,114,182,0.06)] ring-1 ring-slate-100">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 md:space-y-6">
        ...
        <div className="space-y-2">
            ...
            {cardAmount > 0 && (
              <div className="space-y-2">
                {/* ネイティブのみ: App内課金でポイントを購入するページへ誘導 */}
                {isNativeApp && (
                  <Link href={`/points?redirect=/projects/${project.id}`}
                    className="w-full py-3.5 rounded-2xl font-black text-white bg-gradient-to-r from-amber-500 to-orange-500 flex items-center justify-center gap-2 text-sm active:opacity-80 transition-opacity shadow-lg">
