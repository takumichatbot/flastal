          ...
          {calendarDays.map((date, idx) => {
            ...
            return (
              <div key={idx} onClick={() => setSelectedDate(date)}
                className={`aspect-square rounded-xl p-1 cursor-pointer transition-all flex flex-col items-center justify-start relative group border
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3 text-sm text-slate-500 mt-2">
                <span className="flex items-center gap-1"><FiUser/> {offer.project?.planner?.handleName || '匿名'}様</span>
                <span className="flex items-center gap-1"><FiDollarSign/> 予算: {offer.project?.targetAmount?.toLocaleString()}pt</span>
              </div>
            </div>
            <div className="flex items-center">
              <Link href={`/florists/projects/${offer.projectId}`} className="w-full md:w-auto">
                <button className={`w-full px-6 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${type === 'pending' ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-lg shadow-pink-200 hover:-translate-y-0.5' : 'bg-white border border-slate-300 text-slate-600 hover:bg-slate-50'}`}>
                  詳細を確認 <FiArrowRight />
                </button>
              </Link>
            </div>
...
export default function FloristDashboardPage() {
      ...
      return (
        <div className="min-h-screen bg-slate-50 p-4">
            <header className="flex justify-between items-center py-4 px-6 bg-white rounded-xl shadow-sm mb-6">
                <h1 className="font-bold text-gray-700 text-slate-800">Florist Dashboard</h1>
  ...
  const calendarEvents = schedule.map(ev => ({
      ...
      title: ev.project?.title || ev.projectTitle || 'タイトルなし',
      location: ev.venue?.venueName || ev.venueName
  }));
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
             <div className="lg:col-span-2">
                <div className="bg-gradient-to-r from-pink-500 to-rose-500 rounded-2xl p-6 text-white shadow-lg shadow-pink-200 mb-6 flex justify-between items-center relative overflow-hidden">
                    <div className="relative z-10"><h2 className="text-2xl font-bold mb-1">Welcome Back!</h2><p className="opacity-90 text-sm">今日の実績をアピールして、新しいファンを獲得しましょう。</p></div>
             ...
             <div className="space-y-4">
                ...
                <StatCard title="現在の売上残高" value={balance.toLocaleString()} unit="pt" icon={<FiDollarSign size={24} />} color="pink" />
                <Link href="/florists/profile" className="block p-4 bg-slate-800 text-white rounded-2xl text-center hover:bg-slate-700 transition-colors shadow-md">
                    <span className="flex items-center justify-center gap-2 font-bold"><FiSettings /> プロフィール編集</span>
                </Link>
             </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {[
                    ...
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-5 py-3 rounded-xl font-bold text-sm flex items-center gap-2 whitespace-nowrap transition-all ${activeTab === tab.id ? 'bg-white text-pink-600 shadow-md ring-1 ring-pink-100' : 'bg-slate-100 text-slate-500 hover:bg-white hover:text-slate-700'}`}>
                        {tab.icon} {tab.label}
                            ...
                            <div key={post.id} className="group relative aspect-square bg-slate-100 rounded-xl overflow-hidden shadow-sm">
                                ...
                                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3">
                                    <div className="flex justify-end gap-2">
                                        <button onClick={() => handleTogglePost(post)} className="p-2 bg-white/20 backdrop-blur rounded-full text-white hover:bg-white hover:text-slate-800 transition-colors">{post.isPublic ? <FiEye/> : <FiEyeOff/>}</button>
                                        <button onClick={() => handleDeletePost(post.id)} className="p-2 bg-red-500/80 backdrop-blur rounded-full text-white hover:bg-red-600 transition-colors"><FiTrash2/></button>
                                    </div>
                                    <p className="text-white text-xs line-clamp-2">{post.content}</p>
                                </div>
                                {!post.isPublic && <div className="absolute top-2 left-2 bg-slate-800/80 text-white text-[10px] px-2 py-0.5 rounded backdrop-blur">非公開</div>}
                            </div>
                        ))}
                        {appealPosts.length === 0 && <div className="col-span-full py-20 text-center text-slate-400 border-2 border-dashed border-slate-200 rounded-2xl"><FiCamera size={40} className="mx-auto mb-2 opacity-50"/><p>投稿履歴がありません</p></div>}
                    </div>
                )}
            </div>
          </div>
