      ...
      <div className="lg:w-2/3 bg-slate-50/50 p-6 md:p-8 rounded-[2rem] border border-slate-100">
        ...
        <div className="grid grid-cols-7 gap-2">
          {days.map((date, idx) => {
            ...
            const dayEvents = safeEvents.filter(e => e?.date && new Date(e.date).toDateString() === date.toDateString());
              ...
              >
                ...
                <div className="space-y-1 overflow-hidden">
                  {dayEvents.slice(0, 2).map((e, eIdx) => (
                    <div key={e?.id || eIdx} className="text-[9px] font-bold bg-indigo-50 border border-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded-md truncate shadow-sm">
                      {e?.title || '予定'}
                    </div>
                  ))}
                </div>
        ...
        <div className="bg-white/60 p-6 md:p-8 rounded-[2rem] border border-white shadow-sm h-full backdrop-blur-md">
          ...
          <div className="space-y-4 overflow-y-auto max-h-[500px] pr-2 no-scrollbar">
            {selectedEvents.length > 0 ? selectedEvents.map((event, idx) => (
              <Link key={event?.id || idx} href={`/florists/projects/${event?.projectId || event?.id || ''}`} className="block group">
                <div className="p-4 md:p-5 bg-white rounded-2xl border border-slate-100 shadow-sm group-hover:border-pink-300 group-hover:shadow-md transition-all">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-[10px] font-black flex items-center text-slate-500 bg-slate-50 px-2 py-1 rounded-md border border-slate-100">
                        <Clock className="mr-1.5" size={12}/> 
                        {event?.date ? `${new Date(event.date).getHours()}:${String(new Date(event.date).getMinutes()).padStart(2, '0')}` : '時間未定'}
                    </span>
                    <span className="text-[9px] bg-emerald-50 border border-emerald-100 text-emerald-600 px-2 py-1 rounded-full font-black uppercase tracking-widest">
                        {STATUS_LABELS[event?.status] || '進行中'}
                    </span>
                  </div>
                  <h4 className="font-bold text-slate-800 text-sm mb-3 line-clamp-2 group-hover:text-pink-600 transition-colors leading-snug">{event?.title || 'タイトルなし'}</h4>
      ...
      >
        <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center h-14">
          ...
          <div className="flex items-center gap-4">
            <span className="hidden sm:inline-block text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full">{platformName || 'ゲスト'} さん</span>
                ...
                <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}>
                  ...
                  {activeTab === 'pending' && (
                    <div className="space-y-4">
                      {pendingOffers.length > 0 ? pendingOffers.map((o, idx) => (
                  ...
                  {activeTab === 'accepted' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {acceptedOffers.length > 0 ? acceptedOffers.map((o, idx) => (
                        <div key={o?.id || idx} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-lg hover:border-sky-300 hover:-translate-y-1 transition-all group flex flex-col h-full overflow-hidden">
                                      ...
                                      {o?.project?.quotation && (
                                          <div className="flex items-start gap-2.5 pt-2 border-t border-slate-100">
                                              ...
                                              <div>
                                                  ...
                                                  <p className="text-base font-black text-emerald-600">{(o.project?.quotation?.totalAmount || 0).toLocaleString()} <span className="text-xs">pt</span></p>
                                              </div>
                                          </div>
                                      )}
                                  </div>
                  ...
                  {activeTab === 'appeal' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {appealPosts.length > 0 ? appealPosts.map((post, idx) => (
                        <div key={post?.id || idx} className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow group flex flex-col h-full">
