...
import {
  ...
  CheckCircle2, Sparkles, Store, ShieldCheck, Loader2, MapPin
} from 'lucide-react';
...
const STEP_LABELS = ['基本情報', '店舗情報', '配送エリア', '完了'];

function StepIndicator({ currentStep, totalSteps, labels }) {
  return (
    <div className="px-5 py-3 bg-white/60 border-b border-slate-100 rounded-t-[2.5rem] mb-6">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-bold text-slate-500">ステップ {currentStep}/{totalSteps}</span>
        <span className="text-xs font-bold text-pink-500">{labels[currentStep - 1]}</span>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 to-sky-50 flex items-center justify-center p-4 relative overflow-hidden font-sans">
        ...
        <motion.div
          ...
          className="bg-white/80 backdrop-blur-xl max-w-lg w-full border border-white rounded-[2.5rem] shadow-[0_8px_30px_rgba(244,114,182,0.15)] relative z-10 text-center overflow-hidden"
        >
          <StepIndicator currentStep={4} totalSteps={4} labels={STEP_LABELS} />
          <div className="p-8 md:p-10">
            ...
            <p className="text-sm font-bold text-slate-500 mb-6">お花屋さんパートナー登録の第一歩です</p>

            <div className="bg-pink-50 border border-pink-100 rounded-2xl p-4 mb-6">
              <p className="text-sm font-bold text-pink-700 leading-relaxed">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-sky-50 flex items-center justify-center p-4 relative overflow-hidden font-sans pb-24">
      ...
      <motion.div
        ...
        className="bg-white/80 backdrop-blur-xl max-w-lg w-full border border-white rounded-[2.5rem] shadow-[0_8px_30px_rgba(244,114,182,0.15)] relative z-10 mt-8 overflow-hidden"
      >
        {/* プログレスインジケーター */}
        ...
        <div className="p-8 md:p-10">
          <div className="text-center mb-8 relative">
            {currentStep > 1 ? (
              <button
                type="button"
                onClick={handleBack}
                className="absolute left-0 top-0 text-slate-400 hover:text-pink-500 transition-colors p-2 -ml-2 -mt-2 rounded-full hover:bg-pink-50"
              >
                <ArrowLeft size={20} />
              </button>
            ) : (
          ...
          {currentStep === 1 && (
            <form onSubmit={handleNext} className="space-y-5">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2 mb-4 flex items-center gap-1.5"><ShieldCheck size={14}/> 基本情報</h3>
          ...
          {currentStep === 3 && (
            <form onSubmit={handleSubmit} className="space-y-5">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2 mb-4 flex items-center gap-1.5"><MapPin size={14}/> 配送エリア</h3>

              <div>
                <label className="block text-xs font-black text-slate-500 mb-1.5 tracking-widest">対応エリア <span className="text-pink-500">*</span></label>
                <div className="relative">
                  <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                  <input
                    type="text" required name="deliveryArea" value={formData.deliveryArea} onChange={handleChange}
                    placeholder="例: 東京都全域、神奈川県横浜市"
                    className="w-full pl-12 pr-4 py-3.5 bg-white/50 border border-slate-200 rounded-2xl focus:outline-none focus:border-pink-400 focus:ring-4 focus:ring-pink-100 transition-all font-bold text-slate-700 placeholder:text-slate-300"
                  />
                </div>
                <p className="mt-2 text-[10px] font-bold text-slate-400">フラスタ配送が可能なエリアをご記入ください。後から変更も可能です。</p>
              </div>
