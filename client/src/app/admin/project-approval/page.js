...
function ProjectDetailModal({ project, onClose, onAction, isProcessing }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col font-sans">
        ...
        <div className="p-8 overflow-y-auto flex-1 space-y-8 text-slate-800">
          <div>
            <h2 className="text-3xl font-black text-gray-900 mb-4 leading-tight italic">{project.title}</h2>
            <div className="flex flex-wrap gap-6 text-sm font-bold text-gray-500">
               <span className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-xl"><FiUser className="text-pink-500"/> 企画者: {project.planner?.handleName || '不明'}</span>
               <span className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-xl"><FiClock className="text-sky-500"/> 申請日: {new Date(project.createdAt).toLocaleDateString()}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div className="bg-slate-900 p-6 rounded-[2rem] shadow-xl">
                <label className="text-[10px] font-black text-sky-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-2"><FiTarget/> 目標pt</label>
              ...
              <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-100 space-y-4 font-bold">
                <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-1"><FiCalendar/> イベント日</label>
                    <p className="text-slate-800">{project.eventDate ? new Date(project.eventDate).toLocaleDateString() : '未定'}</p>
                </div>
                <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-1"><FiClock/> 募集期限</label>
                    <p className="text-slate-800">{project.deadline ? new Date(project.deadline).toLocaleDateString() : '未定'}</p>
                </div>
              </div>
            </div>

            <div className="rounded-[2rem] overflow-hidden bg-slate-100 border-4 border-white shadow-xl flex items-center justify-center min-h-[250px] relative group">
              {project.imageUrl ? (
                <img src={project.imageUrl} alt="Project" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
              ) : (
                <div className="text-slate-300 flex flex-col items-center">
                  <FiImage size={48} />
                  <span className="text-xs mt-3 font-black uppercase tracking-widest">No Image</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2"><FiFileText/> 企画説明・詳細</label>
        ...
        <div className="p-8 border-t border-gray-100 bg-gray-50 flex gap-5">
          ...
          <button
            ...
            disabled={isProcessing}
            className="flex-[2] bg-slate-900 text-white font-black py-4 rounded-2xl hover:bg-pink-600 shadow-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            <FiCheckCircle /> 承認して募集を開始
          </button>
        </div>
      </div>
    </div>
...
function ProjectApprovalInner() {
  ...
  const fetchPendingProjects = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        setErrorInfo("認証が必要です。ログインし直してください。");
      ...
      if (res.status === 401 || res.status === 403) {
          setErrorInfo("管理者権限エラー。再度ログインしてください。");
          return;
      }
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] p-6 sm:p-12 font-sans text-slate-800 pt-28">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8 px-2 text-slate-800">
            <div className="space-y-4">
                <Link href="/admin" className="inline-flex items-center text-[10px] font-black text-slate-300 hover:text-pink-500 transition-colors uppercase tracking-[0.3em]">
                    <FiArrowLeft className="mr-2"/> 管理トップへ
                </Link>
                <h1 className="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter italic uppercase">Projects</h1>
                <p className="text-slate-400 font-bold text-sm tracking-widest uppercase">新規企画の承認と審査</p>
            </div>
        ...
        {/* 権限エラー表示 */}
        {errorInfo && (
            <div className="mb-12 bg-rose-50 border-2 border-rose-100 p-10 rounded-[3rem] flex flex-col md:flex-row items-center justify-between gap-8 shadow-xl text-slate-800 shadow-rose-500/5">
        ...
        <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 mb-12 flex flex-col md:flex-row items-center gap-8">
            <div className="relative flex-1 w-full group text-slate-800">
                ...
                <input 
                    type="text" 
                    placeholder="企画名や企画者で絞り込む..." 
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full pl-20 pr-10 py-6 bg-slate-50 border-2 border-transparent rounded-[2rem] focus:bg-white focus:border-pink-100 outline-none transition-all font-bold text-xl placeholder:text-slate-200 shadow-sm"
                />
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredProjects.length > 0 ? (
                ...
                <div className="col-span-full py-40 text-center bg-white rounded-[4rem] border-2 border-dashed border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)]">
                    ...
                    <h3 className="text-2xl font-black text-slate-300 italic uppercase tracking-widest">No Projects Pending</h3>
                    <p className="text-slate-300 text-sm mt-3 font-bold uppercase tracking-widest">現在、審査が必要な企画はありません</p>
                </div>
            )}
        </div>
      </div>
...
export default function ProjectApprovalPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-white flex items-center justify-center"><FiLoader className="animate-spin text-pink-500 size-12" /></div>}>
