      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col font-sans">
        ...
        <div className="p-8 overflow-y-auto flex-1 space-y-8 text-slate-800 bg-white">
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">企画内容説明</label>
        ...
        <div className="p-8 border-t border-slate-100 bg-slate-50/80 flex gap-4">
          <button
            ...
            className="flex-1 bg-white border-2 border-slate-200 text-gray-500 font-black py-4 rounded-2xl hover:bg-rose-50 hover:text-rose-500 hover:border-rose-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            <FiXCircle size={20} /> 却下する
          </button>
          <button
  ...
  const handleUpdateStatus = async (projectId, status) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/admin/projects/${projectId}/status`, {
        ...
        body: JSON.stringify({ status }), // ステータス文字列を送信
      });
      ...
      <div className="max-w-7xl mx-auto text-slate-800">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8 px-2 text-slate-800">
            <div className="space-y-4">
                ...
                <h1 className="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter italic uppercase">Regist Review</h1>
                <p className="text-slate-400 font-bold text-sm tracking-widest uppercase">新規プロジェクトの公開承認</p>
            </div>
        ...
        <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 mb-12 flex flex-col md:flex-row items-center gap-8">
            <div className="relative flex-1 w-full group">
                ...
                <input 
                    ...
                    className="w-full pl-20 pr-10 py-6 bg-slate-50 border-2 border-transparent rounded-[2rem] focus:bg-white focus:border-pink-100 outline-none transition-all font-bold text-xl placeholder:text-slate-200 shadow-sm text-slate-800"
                />
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 text-slate-800">
            {filteredProjects.length > 0 ? (
                ...
                <div className="col-span-full py-40 text-center bg-white rounded-[4rem] border-2 border-dashed border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)]">
                    <div className="bg-slate-50 size-24 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-200 shadow-inner text-slate-800">
