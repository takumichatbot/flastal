        ...
        <div className="p-8 overflow-y-auto flex-1 space-y-8 text-slate-800 bg-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-slate-800">
            <div className="space-y-6">
              ...
              <div className="bg-slate-50 p-6 rounded-[2rem] border border-gray-100 space-y-4 font-bold">
                <div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 text-sm text-slate-700 font-medium leading-relaxed whitespace-pre-wrap max-h-80 overflow-y-auto shadow-inner text-slate-800">
...
function ProjectApprovalInner() {
  ...
  // ★ 承認リクエストの本体
  const sendUpdateRequest = async (projectId, status, token) => {
  ...
  const handleUpdateStatus = async (projectId, status) => {
    ...
    const toastId = toast.loading('サーバーへリクエスト送信中...');
    const token = getAuthToken();

    try {
      let res = await sendUpdateRequest(projectId, status, token);

      // 【重要】FUNDRAISING で「無効なステータス」と言われた場合、ACTIVE でリトライする
      if (!res.ok && status === 'FUNDRAISING') {
          console.log("FUNDRAISING failed, retrying with ACTIVE...");
          res = await sendUpdateRequest(projectId, 'ACTIVE', token);
          
          // ACTIVE でもダメなら PUBLISHED でリトライ
          if (!res.ok) {
            console.log("ACTIVE failed, retrying with PUBLISHED...");
            res = await sendUpdateRequest(projectId, 'PUBLISHED', token);
          }
      }
      ...
      if (!res.ok) {
        throw new Error(resData.message || `エラーが発生しました (${res.status})`);
      }

      toast.success(`企画を${actionText}しました`, { id: toastId });
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8 px-2 text-slate-800">
            <div className="space-y-4 text-slate-800">
                <Link href="/admin" className="inline-flex items-center text-[10px] font-black text-slate-300 hover:text-pink-500 transition-colors uppercase tracking-[0.3em] text-slate-800">
                    <FiArrowLeft className="mr-2"/> 管理画面トップ
                </Link>
                <h1 className="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter italic uppercase text-slate-800">Regist Review</h1>
                <p className="text-slate-400 font-bold text-sm tracking-widest uppercase text-slate-800">新規プロジェクトの公開承認</p>
            </div>
            <div className="flex gap-4 text-slate-800">
                <div className="bg-orange-50 px-6 py-4 rounded-[1.5rem] border border-orange-100 flex flex-col items-center shadow-sm text-slate-800">
                    <span className="text-[10px] font-black text-orange-400 uppercase tracking-widest text-slate-800">審査待ち</span>
                    <span className="text-xl font-black text-orange-600 text-slate-800">{projects.length}件</span>
                </div>
                <button onClick={fetchPendingProjects} className="p-6 bg-white border border-slate-100 rounded-[1.5rem] hover:bg-slate-50 shadow-sm text-slate-400 transition-all active:rotate-180 duration-500 text-slate-800">
            ...
            <div className="mb-12 bg-rose-50 border-2 border-rose-100 p-10 rounded-[3rem] flex flex-col md:flex-row items-center justify-between gap-8 shadow-xl text-slate-800 shadow-rose-500/5">
                <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                        <p className="font-black text-rose-900 text-xl tracking-tight italic">エラーが発生しました</p>
                        <p className="text-rose-700/60 text-sm font-bold uppercase tracking-widest leading-relaxed">{errorInfo}</p>
                    </div>
                </div>
                <button onClick={() => { logout(); router.push('/login'); }} className="px-10 py-5 bg-rose-500 text-white rounded-[1.5rem] font-black text-xs uppercase hover:bg-rose-600 transition-all shadow-lg shadow-rose-200 active:scale-95 text-slate-800">再ログインして修復</button>
            </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 text-slate-800">
            {filteredProjects.length > 0 ? (
                    ...
                    <div key={project.id} className="bg-white rounded-[3rem] p-8 border-2 border-slate-50 transition-all flex flex-col justify-between group hover:border-pink-50 hover:shadow-[0_30px_60px_rgba(0,0,0,0.03)]">
                        <div>
                            ...
                            <div className="space-y-2 mb-8 text-xs font-bold text-slate-400">
                                ...
                                <p className="flex items-center gap-3 font-black text-slate-800"><FiTarget className="text-sky-500"/> 目標額: <span className="text-slate-800 font-black">{Number(project.targetAmount).toLocaleString()} pt</span></p>
                            </div>
                        </div>
                ...
                <div className="col-span-full py-40 text-center bg-white rounded-[4rem] border-2 border-dashed border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)]">
                    ...
                    <p className="text-slate-300 text-sm mt-3 font-bold uppercase tracking-widest">現在、すべての企画の処理が完了しています</p>
                </div>
            )}
        </div>
      </div>
