...
import { 
    ...
    MoreVertical, ShieldCheck, Palette, Store, Building2, User, RefreshCw
} from 'lucide-react';
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="py-4 flex items-center justify-between">
                        ...
                        <div className="text-sm font-bold text-slate-500 bg-slate-100 px-4 py-2 rounded-full">
                            合計: {filteredUsers.length} アカウント
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
                    ...
                    <div className="relative w-full md:w-96 group">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/50 focus:border-sky-500 transition-all font-medium"
                        />
                    </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto scrollbar-hide">
                        {[
                            ...
                            <button
                                ...
                                className={cn(
                                    "px-4 py-2 text-xs font-bold rounded-xl whitespace-nowrap transition-all duration-200",
                                    activeTab === tab.id 
                                    ...
                                    filteredUsers.map((u) => {
                                        ...
                                        return (
                                            <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                                                ...
                                                {/* 権限・ロール */}
                                                <td className="px-6 py-4">
                                                    <span className={cn("inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border", badge.bg, badge.text, badge.bg.replace('bg-', 'border-'))}>
                                                ...
                                                {/* アクション (将来的な詳細ページ等への導線) */}
                                                <td className="px-6 py-4 text-right">
                                                    <button className="p-2 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition-colors">
