...
export default function AdminContactPage() {
    ...
    // フォーム状態
    const [targetUser, setTargetUser] = useState(null); // 選択されたユーザーオブジェクト
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            ...
            setIsSearching(true);
            try {
                // ※ エンドポイントは環境に合わせて実装してください (例: /api/admin/users?q=...)
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`${API_URL}/api/admin/contact/send`, {
                ...
                body: JSON.stringify({ 
                    userId: targetUser.id, // IDで紐付けるのが確実
                        ...
                        <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                            ...
                            <div className="relative">
                                <input 
                                    ...
                                    className="w-full p-3 pl-10 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all"
                                    placeholder="名前、メールアドレス..."
                                    disabled={!!targetUser} // 選択中はロック
                                />
                                <FiSearch className="absolute left-3 top-3.5 text-slate-400" />
                                
                                {targetUser && (
                                    <button 
                                        onClick={() => { setTargetUser(null); setSearchTerm(''); setSearchResults([]); }}
                                ...
                                <div className="mt-2 bg-white border border-slate-200 rounded-lg shadow-lg max-h-60 overflow-y-auto absolute z-10 w-[calc(100%-2.5rem)] lg:w-72">
                                    {isSearching ? (
                                        ...
                                        searchResults.map(user => (
                                            <button 
                                                key={user.id}
                                                onClick={() => { setTargetUser(user); setSearchTerm(user.email); }}
                                                className="w-full text-left p-3 hover:bg-indigo-50 border-b border-slate-100 last:border-0 flex items-center gap-3 transition-colors"
                                            >
                            ...
                            {targetUser && (
                                <div className="mt-4 p-4 bg-indigo-50 border border-indigo-100 rounded-lg animate-fadeIn">
                                    ...
                                    <p className="text-xs text-indigo-600 bg-white p-2 rounded border border-indigo-100 break-all">
                                        {targetUser.email}
                                    </p>
                            ...
                            <div className="space-y-2">
                                {TEMPLATES.map((tpl, idx) => (
                                    <button
                                        ...
                                        className="w-full text-left text-xs p-2.5 bg-slate-50 hover:bg-slate-100 rounded border border-slate-200 text-slate-700 transition-colors"
                                    >
                                        {tpl.label}
                                    </button>
                                ))}
                            ...
                            <form onSubmit={handleSend} className="flex-1 flex flex-col space-y-6">
                                ...
                                <div className="flex-1 flex flex-col">
                                    ...
                                    <textarea 
                                        ...
                                        className="w-full flex-1 p-4 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none resize-none bg-slate-50 text-slate-800 leading-relaxed min-h-[300px]"
                                ...
                                <div>
                                    ...
                                    <div className="flex items-start gap-4">
                                        <button 
                                            ...
                                            className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 text-slate-600 transition-colors shadow-sm"
                                        >
                                            <FiPaperclip /> ファイルを選択
                                        </button>
                                        <input 
                                        ...
                                        {selectedFile && (
                                            <div className="relative group">
                                                {previewUrl ? (
                                                    <div className="w-16 h-16 rounded-lg overflow-hidden border border-slate-200 relative">
                                                        <Image src={previewUrl} alt="Preview" fill className="object-cover" />
                                                    </div>
                                                ) : (
                                                    <div className="flex items-center gap-2 text-sm text-slate-600 bg-slate-100 px-3 py-2 rounded">
                                                        <FiImage /> {selectedFile.name}
                                                    </div>
                                                )}
                                                <button
                                                    type="button"
                                    ...
                                    <p className="text-xs text-slate-400 mt-2">※ 5MB以下の画像(JPG, PNG)またはPDF</p>
                                </div>
