...
export default function AdminVenuesPage() {
  ...
  const fetchVenues = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
          setErrorInfo("管理者権限エラー。一度ログアウトし、再ログインしてください。");
          return;
      }
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // ここで失敗理由を表示
        toast.error(`承認失敗: ${err.message || '権限不足'}`, { duration: 5000 });
      }
    } catch (error) {
      toast.dismiss(loadingToast);
  ...
  if (authLoading) return <div className="min-h-screen bg-white flex items-center justify-center font-sans text-slate-800"><FiLoader className="animate-spin text-pink-500 size-10" /></div>;
      ...
      <div className="max-w-7xl mx-auto text-slate-800">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-8 px-2">
            <div className="space-y-4">
                ...
                <p className="text-slate-400 font-bold text-sm tracking-[0.2em] uppercase">承認・整理</p>
            </div>
            <div className="flex gap-4">
                <button onClick={fetchVenues} className="p-5 bg-white border border-slate-100 rounded-2xl hover:bg-slate-50 shadow-sm text-slate-400">
                    <FiRefreshCw className={loadingData ? 'animate-spin' : ''} />
                </button>
                <button onClick={() => { setEditingVenue(null); setIsModalOpen(true); }} className="flex items-center gap-3 bg-slate-900 text-white px-10 py-5 rounded-[2rem] hover:bg-pink-600 shadow-2xl transition-all font-black active:scale-95 group text-sm uppercase tracking-widest">
        ...
        {errorInfo && (
            <div className="mb-12 bg-rose-50 border-2 border-rose-100 p-10 rounded-[3rem] flex flex-col md:flex-row items-center justify-between gap-8 text-slate-800 shadow-[0_20px_40px_rgba(225,29,72,0.05)]">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-rose-700/60 text-sm font-bold mt-1 uppercase tracking-widest leading-relaxed">{errorInfo}</p>
                    </div>
                </div>
                <button onClick={() => { logout(); router.push('/login'); }} className="px-10 py-5 bg-rose-500 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest hover:bg-rose-600 transition-all shadow-lg shadow-rose-100">再ログインする</button>
            </div>
        )}
        ...
        <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 mb-12 flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="px-12 py-6 bg-slate-50 rounded-[2rem] text-[10px] font-black text-slate-300 tracking-[0.3em] border border-slate-100/50 whitespace-nowrap">
        ...
        <div className="space-y-6">
          {loadingData && !errorInfo ? (
            ...
            <div className="bg-white rounded-[3rem] py-40 text-center border-2 border-dashed border-slate-50 text-slate-300 font-black italic">データが見つかりません</div>
          ) : (
            filteredVenues.map((venue) => (
                <div key={venue.id} className={`bg-white rounded-[3rem] p-10 border-2 transition-all flex flex-col md:flex-row items-center gap-10 group ${!venue.isOfficial ? 'border-pink-200 bg-pink-50/10 shadow-[0_20px_50px_rgba(244,114,182,0.1)]' : 'border-slate-50 hover:border-pink-50'}`}>
                    <div className="flex-1 w-full text-slate-800">
                        <div className="flex flex-wrap items-center gap-4 mb-4">
                            <h3 className="font-black text-slate-800 text-3xl tracking-tighter italic uppercase">{venue.venueName}</h3>
                            {!venue.isOfficial && (
                                <span className="bg-pink-500 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 animate-pulse shadow-lg shadow-pink-200"><FiClock /> 承認待ち</span>
                            )}
                        </div>
