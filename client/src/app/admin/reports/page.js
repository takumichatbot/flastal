...
function AdminReportsInner() {
  ...
  const handleReportAction = async (report, action) => {
    ...
    if (action === 'DISMISS') {
        ...
        else if (activeTab === 'project') endpoint = `/api/admin/projects/${report.id}/dismiss`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8 font-sans text-gray-800">
        ...
        <div className="flex gap-2 mb-0 border-b border-gray-200">
          {[
            ...
            <button 
                ...
                className={`px-6 py-3 rounded-t-xl font-bold text-sm flex items-center gap-2 transition-all relative top-[1px]
                    ${activeTab === tab.id ? 'bg-white text-indigo-600 border border-gray-200 border-b-white shadow-sm z-10' : 'bg-gray-100 text-gray-500 border-transparent hover:bg-gray-200'}`}
            >
                {tab.icon} {tab.label}
                {tab.count > 0 && <span className={`ml-1 text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-red-100 text-red-600' : 'bg-gray-300 text-white'}`}>{tab.count}</span>}
            </button>
          ))}
        </div>
            ...
            {currentList.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-gray-50 text-xs text-gray-500 font-bold tracking-wider border-b border-gray-100">
                            <tr>
                        ...
                        <tbody className="divide-y divide-gray-100 text-sm">
                            {currentList.map((report) => {
                                let contentPreview = activeTab === 'chat' ? report.content : activeTab === 'event' ? report.event?.title : report.project?.title;
                                return (
                                    <tr key={report.id} className="hover:bg-indigo-50/30 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-700 border border-red-100">{report.reason}</span>
                                        </td>
                                        <td className="px-6 py-4 max-w-xs">
                                            <div className="font-bold text-gray-800 truncate">{contentPreview || <span className="text-gray-400 italic">不明</span>}</div>
      ...
      <ReportDetailModal report={selectedReport} type={activeTab.toUpperCase()} onClose={() => setSelectedReport(null)} onAction={handleReportAction} />
    </div>
  );
}
