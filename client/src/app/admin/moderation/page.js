...
import { Search, MessageSquare, User, Filter, AlertCircle, CheckCircle2, Clock, Activity } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h1 className="text-2xl font-extrabold text-gray-900 flex items-center gap-2">
              <MessageSquare className="text-sky-500"/> チャットルーム監視
            </h1>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden min-h-[400px]">
          <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
            <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2">
              <Activity className="text-sky-500"/> 監視対象プロジェクト
          ...
          <div className="p-6">
            {loadingData ? (
              ...
              <div className="text-center py-20 border-2 border-dashed border-gray-100 rounded-xl">
                <MessageSquare className="mx-auto text-4xl text-gray-300 mb-3" />
...
function StatusBadge({ status }) {
  const config = {
    RECRUITING: { color: 'bg-green-100 text-green-700', icon: <CheckCircle2 />, label: '募集中' },
    IN_PROGRESS: { color: 'bg-blue-100 text-blue-700', icon: <Activity />, label: '進行中' },
    COMPLETED: { color: 'bg-gray-100 text-gray-600', icon: <Clock />, label: '完了' },
    DRAFT: { color: 'bg-yellow-100 text-yellow-700', icon: <AlertCircle />, label: '下書き' },
  };
