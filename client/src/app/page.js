    ...
    <div className="container mx-auto px-0 md:px-6 max-w-7xl relative z-10">
      ...
      <div className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar pb-8 px-6 md:px-0 md:grid md:grid-cols-4 gap-4 md:gap-6">
        {DUMMY_PROJECTS.map((project, i) => (
          <ProjectCardWrapper key={project.id} project={project} index={i} />
        ))}
      </div>
    </div>
  </section>
                ...
                <div className="flex items-center gap-4 mb-4 md:mb-6">
                  ...
                  <div>
                    <span className={cn("text-[9px] md:text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider", 
                      v.color === "pink" ? "bg-pink-100 text-pink-600" : v.color === "sky" ? "bg-sky-100 text-sky-600" : "bg-emerald-100 text-emerald-600"
                    )}>
                      {v.role}
                    </span>
...
const PartnerBanner = () => (
  <section className="py-12 md:py-20 mx-4 md:mx-auto max-w-6xl mb-12">
    <Reveal>
      <div className="bg-slate-900 rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-16 text-center relative overflow-hidden shadow-2xl border-4 border-slate-800">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-white/10 rounded-2xl backdrop-blur-md mb-6 border border-white/20"><Store className="text-white" size={32} /></div>
          <h2 className="text-xl md:text-3xl font-black text-white mb-4">クリエイター・法人の皆様へ</h2>
          <p className="text-slate-300 text-xs md:text-sm mb-10 max-w-2xl mx-auto leading-relaxed">
    ...
    <Reveal className="relative z-10 max-w-4xl mx-auto">
      ...
      <Link href="/projects/create" className="inline-block w-full sm:w-auto">
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
          className="w-full sm:w-auto bg-gradient-to-r from-pink-500 to-rose-500 text-white px-8 md:px-14 py-4 md:py-6 rounded-full text-base md:text-xl font-black shadow-xl flex items-center justify-center gap-2 md:gap-3 mx-auto"
        >
