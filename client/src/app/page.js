...
/**
 * ダッシュボード本体（ビルド時には解析されないようにガードを最大にする）
 */
function AdminDashboardInner() {
  ...
  // マウント前は一切のロジックを実行せずローディングを表示
  if (!isMounted) return <AdminLoading message="Initializing System..." />;
...
export default function AdminPage() {
  return (
    <Suspense fallback={<AdminLoading message="Starting..." />}>
      <AdminDashboardInner />
    </Suspense>
  );
}