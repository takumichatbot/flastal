...
import { useAuth } from '@/app/contexts/AuthContext'; // パスは @/app/... で絶対パス指定が確実です
...
export default function FloristAppealPostForm({ onPostSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ここで authenticatedFetch を使うことでトークンが確実に送られます
      ...
      if (!presignedRes.ok) {
        ...
        throw new Error(error.message || 'アップロード許可の取得に失敗しました');
      }
      
      const { uploadUrl, fileUrl } = await presignedRes.json();

      // 2. S3へ直接アップロード (ここは通常のfetch)
      ...
      // 3. APIへ投稿データを送信 (authenticatedFetchを使用)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-pink-100 overflow-hidden mb-8">
        ...
        <form onSubmit={handleSubmit} className="p-5">
            ...
            >
                {isUploading ? <><FiLoader className="animate-spin text-xl"/> 公開中...</> : <><FiSend className="text-lg"/> ギャラリーに公開する</>}
            </button>
        </form>
    </div>
  );
