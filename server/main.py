...
@app.post("/api/token", description="ファンユーザーのログイン")
...
@app.get("/api/venues", response_model=list[schemas.VenuePublic])
...
def get_chat_room_details(chat_room_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    if not chat_room or (chat_room.project.planner_id != current_user.id and chat_room.florist.id != current_user.id): # 簡易的な権限チェック
        raise HTTPException(status_code=404, detail="Chat room not found or not authorized")
    return chat_room
...
async def handle_chat_message(sid, data):
    ...
    try:
        ...
        message_for_client = schemas.ChatMessage.from_orm(new_message).dict()
